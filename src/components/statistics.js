...
const getTasksByDateRange = (tasks, dateFrom, dateTo) => tasks.filter((task) => task.dueDate >= dateFrom && task.dueDate <= dateTo);

const createRandomColor = () => {
  const value = Math.floor(Math.random() * (0xffffff + 1));
  ...
  return new Chart(colorsCtx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: colors.map((color) => COLOR_VALUE[color])
      }]
    },
    options: {
      plugins: {
...
const renderDaysChart = (daysCtx, tasks, dateFrom, dateTo) => {
  ...
  const taskCountOnDay = days.map((date) =>
    tasks.filter((task) => isOneDay(task.dueDate, date)).length
  );
